...
class MethodChannelVideoPlayerWin extends VideoPlayerWinPlatform {
  ...
  Future<WinVideoPlayerValue?> openVideo(
    ...
    playerMap[value.textureId] = WeakReference<WinVideoPlayerController>(
      player,
    );
    return value;
  }
  ...
  Future<void> seekTo(int textureId, int ms) async {
    ...
    await methodChannel.invokeMethod<bool>('seekTo', {
      "textureId": textureId,
      "ms": ms,
    });
  }
  ...
  Future<int> getCurrentPosition(int textureId) async {
    ...
    var value = await methodChannel.invokeMethod<int>('getCurrentPosition', {
      "textureId": textureId,
    });
    return value ?? -1;
  }
  ...
  Future<int> getDuration(int textureId) async {
    var value = await methodChannel.invokeMethod<int>('getDuration', {
      "textureId": textureId,
    });
    return value ?? -1;
  }
  ...
  Future<void> setPlaybackSpeed(int textureId, double speed) async {
    await methodChannel.invokeMethod<bool>('setPlaybackSpeed', {
      "textureId": textureId,
      "speed": speed,
    });
  }
  ...
  Future<void> setVolume(int textureId, double volume) async {
    await methodChannel.invokeMethod<bool>('setVolume', {
      "textureId": textureId,
      "volume": volume,
    });
  }
