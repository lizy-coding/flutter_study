...
class WinVideoPlayerController extends ValueNotifier<WinVideoPlayerValue> {
  ...
  WinVideoPlayerController.network(
    String dataSource, {
    bool isPluginMode = false,
    Map<String, String> httpHeaders = const {},
  }) : this._(
         dataSource,
         WinDataSourceType.network,
         isPluginMode: isPluginMode,
         httpHeaders: httpHeaders,
       );
  WinVideoPlayerController.networkUrl(
    Uri url, {
    bool isPluginMode = false,
    Map<String, String> httpHeaders = const {},
  }) : this._(
  ...
  WinVideoPlayerController.asset(
    String dataSource, {
    String? package,
    bool isPluginMode = false,
  }) : this._(
         getAssetPath(dataSource),
         WinDataSourceType.file,
         isPluginMode: isPluginMode,
       );
  ...
  void _startTrackingPosition() async {
    ...
    _positionTimer = Timer.periodic(const Duration(milliseconds: 300), (
      Timer timer,
    ) async {
  ...
  Future<void> initialize() async {
    WinVideoPlayerValue? pv = await VideoPlayerWinPlatform.instance.openVideo(
      this,
      textureId_,
      dataSource,
      httpHeaders,
    ...
    if (pv == null) {
      ...
      value = value.copyWith(
        isInitialized: false,
        errorDescription: "open file failed",
      );
      _eventStreamController.add(
        VideoEvent(
  ...
  Future<void> seekTo(Duration time) async {
    ...
    await VideoPlayerWinPlatform.instance.seekTo(
      textureId_,
      time.inMilliseconds,
    );
  ...
  Future<int> _getCurrentPosition() async {
    ...
    int pos = await VideoPlayerWinPlatform.instance.getCurrentPosition(
      textureId_,
    );
